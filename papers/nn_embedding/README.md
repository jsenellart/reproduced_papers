...
For the reimplementation, the photonic embedding takes more parameters than the gate-based model since the number of parameters generated by the ``CircuitBuilder``'s ``add_entangling_layer``is not defined by the user.
...
- **Notes:** Always using the ADAM optimiser (the paper uses Nesterov). There is no specific reason why this optimizer was chosen. The ADAM optimizer is often better than Nestorov for noisy gradients. Since the cost landscape is not discussed here, we chose to use ADAM to have the better results. If the cost landscape is indeed well behaved with non-noisy gradients, the Nesterov optimizer would converge faster. 

In brief, ADAM was used to ensure a good performance baseline on our models.
...
- The results are stored in the outdir folder. Logs and figures will be saved in the outdir directory.
...
### MerLin implementation notes
The MerLin implementation is similar to the gate based one. Here is the main implementation notes.
- The NQE model takes two ``QuantumLayer``s as input (the quantum embedder and quantum classifier) as well as a classical model.
- In the constructor:
...
Figures 2 through 6 are reproduced using noise-free simulations with Pennylane. Some better or equivalent results are observed with the MerLin implementation.
...
![](outdir/figure_2/figure_2_bc_gate_based.png)

#### MerLin reproduction
![](outdir/figure_2/figure_2_bc_merlin.png)

#### Discussion
We observe the same trends than the original paper for the gate and MerLin based reproductions. Although for the MerLin implementation, the normal encoded states are more distant maybe explaining the fact the the NQE without PCA encodes better states at earlier iterations. The model without the embedding training also performs better than the gate-based reproduction who is way worse than the NQE model from the paper. It is maybe explained by the fact that the loss does not reach the lower bound as much as it is shown on the paper.
...
#### Gate-based reproduction
![](outdir/figure_3/figure_3_gate_based.png)

#### MerLin reproduction
![](outdir/figure_3/figure_3_merlin.png)
...
For more details about the parameters of the local effective dimension and the way it is computed here. Checkout section 4 in this [LaTeX document](https://www.overleaf.com/read/dtcyrcpybygv#da142d). 

#### Paper’s result
![](img_paper/fig_4_paper.png)

#### Gate-based reproduction
![](outdir/figure_4/figure_4_gate_based.png)

#### MerLin reproduction
![](outdir/figure_4/figure_4_merlin.png)
...
#### Gate-based reproduction
![](outdir/figure_5/figure_5_gate_based.png)

#### MerLin reproduction
![](outdir/figure_5/figure_5_merlin.png)
...
$$\epsilon=\sqrt{\text{Tr}(A^\dagger A)}$$
Where
...
#### Gate-based reproduction
![](outdir/figure_6/figure_6_gate_based.png)

#### MerLin reproduction
![](outdir/figure_6/figure_6_merlin.png)
...
- Obtain the same or better results than the gate-based model. Indeed, in figure 3 we  observed a bigger advantage of the MerLin NQE model versus trainable unitaries and figure 5 was reproduced compared to the gate-based version.
...
- The embedding and classifier circuits are a trainable universal unitary with the add_entangling_layer method of the CircuitBuilder.

### 7.1. MerLin feasibility

#### Can this be done in MerLin? 

- The entire pipeline was reproduced in MerLin and seems to be faster than the Pennylane implementation. 
