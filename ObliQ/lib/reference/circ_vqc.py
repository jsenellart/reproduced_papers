...
class PhotoCircuit:
    ...
    def build_circuit2(self, Q, coeffs):
        """Builds the photonic circuit based on the QUBO matrix Q and coefficients."""
        circ = pcvl.Circuit(self.size)
        ...
        for _ in range(num_rep):
            for i in range(self.size):
                for j in range(i + 1, self.size):
                    ...
                    root_qij = Q_normalized[i, j]/(num_rep**2)
    ...
    def forward2(self, Q, ones, zeros, coeffs):
        """Simulate the forward pass of the photonic circuit."""
        circ = self.build_circuit2(Q, coeffs)
        input_state = self.generate_input(sum(ones), self.size, ones)

        QPU = pcvl.Processor("CliffordClifford2017", circ) if self.real_machine == 0 else pcvl.RemoteProcessor(self.backend, self.token)
        QPU.with_input(input_state)
        QPU.min_detected_photons_filter(1)
    ...
    def test_model2(self, Q, ones, zeros, coeffs):
        """Test the photonic circuit model."""
        probs1 = self.forward2(Q, ones, zeros, coeffs)
        avg_num_1 = np.zeros(self.size)
        probs_val = 0
        largest_vec = np.zeros(self.size)
